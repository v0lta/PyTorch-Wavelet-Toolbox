...
def test_fwt_ifwt_level_2():
    ...
    print(np.mean(np.abs(coeffs2[0] - coeffsmat2[0].squeeze().numpy()))
          < 1e-6,
          np.mean(np.abs(coeffs2[1] - coeffsmat2[1].squeeze().numpy()))
          < 1e-6,
          np.mean(np.abs(coeffs2[2] - coeffsmat2[2].squeeze().numpy()))
          < 1e-6)
    assert err1 < 1e-6
    assert err2 < 1e-6
    assert err3 < 1e-6
