...
class WaveletPacket2D(collections.UserDict):
    ...
    def _wavepacketdec2d(self, data, wavelet, mode, max_level=None):
        self.data = {}
        if max_level is None:
            max_level = pywt.dwt_max_level(min(self.input_data.shape[2:]),
                                           self.wavelet)
        self.recursive_dwt2d(self.input_data, level=0, max_level=max_level,
                             path='')

    def recursive_dwt2d(self, data, level, max_level, path):
        self.data[path] = data
        if level < max_level:
            resa, (resh, resv, resd) = wavedec2(data, self.wavelet, level=1,
                                                mode=self.mode)
            return self.recursive_dwt2d(resa, level+1, max_level, path + 'a'),\
...
if __name__ == '__main__':
    ...
    print('h', np.mean(np.abs(wp_tree['h'].data
          - torch.squeeze(ptwt_wp_tree['h']).numpy())))
    print('v', np.mean(np.abs(wp_tree['v'].data
