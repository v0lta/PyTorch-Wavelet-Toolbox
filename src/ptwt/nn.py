...
class WaveletLayer(torch.nn.Module):
    """A learnable wavelet layer based on https://arxiv.org/pdf/2004.09569.pdf.

    Create a learnable Wavelet layer as described here:

    The weights are parametrized by S*W*G*P*W*B, with S, G, B diagonal matrices;
    P a random permutation; and W a learnable wavelet transform.
    ...
    def _wavelet_analysis(self, x: torch.Tensor) -> torch.Tensor:
        """Compute a 1D-analysis transform on a 2D input tensor."""
    ...
    def _wavelet_reconstruction(self, x: torch.Tensor) -> torch.Tensor:
        """Reconstruct an input tensor from an analysis coefficient tensor."""
        coeff_lst = []
        start = 0
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        step1 = self._mul_b(x)
        step2 = self._wavelet_analysis(step1)
        step3 = self._mul_p(step2)
        step4 = self._mul_g(step3)
        step5 = self._wavelet_reconstruction(step4)
    ...
    def get_wavelet_loss(self) -> torch.Tensor:
        """Return the wavelet loss for the wavelet in the layer.

        This value must be added to the cost for the wavelet learning to work.
